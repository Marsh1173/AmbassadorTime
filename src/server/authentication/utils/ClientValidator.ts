...
export interface ValidateLoginSuccess extends Success {
  user_data: UserData;
}
export type ValidateLoginReturnMsg = FailureMsg | ValidateLoginSuccess;

export interface IClientValidator {
  attempt_validate_client_login(user_id: string, password: string): ValidateLoginReturnMsg;
}

export class ClientValidator implements IClientValidator {
  constructor(private readonly auth_service: IAuthenticationService) {}

  public attempt_validate_client_login(user_id: string, password: string): ValidateLoginReturnMsg {
    //validate user id and password
    let validate_user_id_and_password: ValidateLoginReturnMsg = this.auth_service.server_app.user_dao.validate_login({
      id: user_id,
      password,
    });
    ...
    let already_logged_in: ReturnMsg = this.auth_service.authenticated_client_tracker.attempt_add_client(user_id);

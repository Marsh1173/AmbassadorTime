...
export class ActionHistoryView extends Component<ActionHistoryViewProps, ActionHistoryViewState> {
  constructor(props: ActionHistoryViewProps) {
    ...
    this.state = { month: date.getMonth(), year: date.getFullYear(), action_log: "loading" };
  ...
  public render() {
    ...
    if (this.state.action_log === "loading") {
      ...
      action_log_elem = <span className="none">None found for this month.</span>;
    } else {
      action_log_elem = (
        <div className="action-log-container">
          {this.state.action_log.split("\n").map((line) => {
            return <span className="action-log">{line}</span>;
          })}
        </div>
      );
    }
    ...
    return (
      <div className="ActionHistoryView">
        <div className="header">
          ...
          <span>{month_to_string(this.state.month) + ", " + this.state.year}</span>
